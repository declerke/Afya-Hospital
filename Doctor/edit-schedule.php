...
// Fetch the schedule data (ensure it belongs to the logged-in doctor)
$schedule = null;
try {
    $stmt = $pdo->prepare("
        SELECT ds.*
    ...
    ");
    $stmt->execute([$schedule_id, $doctorScheduleId]);
    $schedule = $stmt->fetch(PDO::FETCH_ASSOC);

    if (!$schedule) {
        die("Schedule not found or you do not have permission to edit this schedule.");
    }
} catch (PDOException $e) {
